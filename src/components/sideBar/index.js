    ...
    return (
        <>
            <div class={props.active ? "sidebar" : "sidebar collapsed"}  >
                ...
                <div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper"  style={{height: `calc((100vh - 72px) - 2rem)`, marginRight:"4px"}}>
                    <div id="kt_aside_menu" class="aside-menu my-4 scroll ps ps--active-y "  data-menu-vertical="1" data-menu-scroll="1" data-menu-dropdown-timeout="500" style={{ height: "100%",overflow:props.active?"auto":"hidden"}} >
                        <ul class="menu-nav" >
                            {tests.map(test=>{ return  (<li  class="menu-item" aria-haspopup="true">
                                <a class="menu-link" onClick={() => scrollToElement(test.uuid)}>
                                    <span class="svg-icon menu-icon" >{test.state=="passed" ? <IcCheck />:<IcError/>}</span>
                                    <span  style={props.active ? { display: 'block' } : { display: 'none' }} class="menu-text">{test.title}</span>
                                </a>
                                </li>)})}
                        </ul>       
                    </div>
